...
for i in range(1, stop = length(Betas))
    for j in range(1, stop = length(Epsilons))
        for k in range(1, stop = length(Severe_Prob))
            local param_int = [c, Betas[i], gamma, alpha, Severe_Prob, gamma_s, Epsilons[j], phi]
...
println("Beta, epsilon, and p_s values that minimise infected error: $(Betas[current_min_indices_infected[1]]), $(Epsilons[current_min_indices_infected[2]]), $(Severe_Prob[current_min_indices_infected[3]])")
println("Beta, epsilon, and p_s values that minimise severe illness error: $(Betas[current_min_indices_severe[1]]), $(Epsilons[current_min_indices_severe[2]]), $(Severe_Prob[current_min_indices_severe[3]])")
