...
#Model the disease in the second town using the LLSE approximated values obtained from
# Project_TLT5_MultiVariableErrorPlotting.jl
...
gamma_s = 1/10
p_s = measurement(0.2, 0.05) # Average probability of severe infection
alpha = 1/30 # Daily rate of resusceptance if the average time for it is a month
epsilon = 0.05 # Efficacy of intervention
phi = 0.59 # Proportion of population that will adhere to the intervention

#Estimated Beta that minimises error with the data
Beta = measurement(0.0326, 0.002)
...
plot!(main, range(27, stop = 80, step = 1), town2_Infected_d27_d80, label = "Infected population in town 2 data as of 31/10",  seriestype=:scatter)
display(main)

#Create a plot for the severe illness model and data
severe = plot()
...
plot!(severe, range(27, stop = 80, step = 1),town2_Severe_d27_d80, label = "Severe Illness Data in Town 2 as of 31/10",  seriestype=:scatter)
display(severe)
display(main)